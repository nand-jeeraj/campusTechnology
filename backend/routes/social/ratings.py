...
@router.post("/rate")
async def submit_rating(rating_data: RatingSubmit):
    user = get_dummy_user()
...
async def get_ratings(faculty_id: str = Path(..., description="The ID of the faculty to get ratings for")):
    user = get_dummy_user()
    try:
...
async def submit_course_rating(course_rating: CourseRatingSubmit):
    user = get_dummy_user()
    try:
...
async def view_all_course_ratings():
    user = get_dummy_user()
    if user["role"] != "faculty":
        raise HTTPException(status_code=403, detail="Only faculty can view all course ratings")
    try:
...
async def get_faculty_course_ratings():
    user = get_dummy_user()
    if user["role"] != "student":
        raise HTTPException(status_code=403, detail="Only faculty can view course ratings")
    try:
        ratings = list(course_ratings_collection.find())
        result = [{
